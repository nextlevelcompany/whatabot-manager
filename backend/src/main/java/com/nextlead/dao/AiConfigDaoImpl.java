...
public class AiConfigDaoImpl implements AiConfigDao {
    ...
    public List<ShippingCoverage> getAllShippingCoverage() {
        String sql = "SELECT id, district_name, delivery_fee, min_order_amount, is_active FROM shipping_coverage ORDER BY district_name ASC";
    ...
    public void saveShippingCoverage(ShippingCoverage coverage) {
        if (coverage.getId() == null) {
            String sql = "INSERT INTO shipping_coverage (district_name, delivery_fee, min_order_amount, is_active) VALUES (?, ?, ?, ?) " +
                         "ON CONFLICT (district_name) DO UPDATE SET delivery_fee = EXCLUDED.delivery_fee, min_order_amount = EXCLUDED.min_order_amount, is_active = EXCLUDED.is_active";
            jdbcTemplate.update(sql, coverage.getDistrictName(), coverage.getDeliveryFee(), coverage.getMinOrderAmount(), coverage.getIsActive());
        } else {
            String sql = "UPDATE shipping_coverage SET district_name = ?, delivery_fee = ?, min_order_amount = ?, is_active = ? WHERE id = ?";
            jdbcTemplate.update(sql, coverage.getDistrictName(), coverage.getDeliveryFee(), coverage.getMinOrderAmount(), coverage.getIsActive(), coverage.getId());
        }
    }
    ...
    public List<AiProductConfig> getAllAiProductsConfig() {
        // Enlaza la tabla de productos con la tabla de configuración de IA. Si no existe configuración, devuelve valores por defecto.
        String sql = "SELECT a.id, p.id AS producto_id, COALESCE(a.ai_enabled, TRUE) AS ai_enabled, " +
                     "COALESCE(a.search_keywords, p.nombre) AS search_keywords, a.custom_ai_description, " +
                     "p.codigo, p.nombre, p.precio_venta, p.imagen " +
                     "FROM productos p LEFT JOIN ai_products_config a ON p.id = a.producto_id WHERE p.activo = TRUE ORDER BY p.nombre ASC";
    ...
    public void saveAiProductConfig(AiProductConfig config) {
        String sql = "INSERT INTO ai_products_config (producto_id, ai_enabled, search_keywords, custom_ai_description) " +
                     ...
                     "custom_ai_description = EXCLUDED.custom_ai_description";
        jdbcTemplate.update(sql, config.getProductoId(), config.getAiEnabled(), config.getSearchKeywords(), config.getCustomAiDescription());
    }
    ...
    public List<AiKnowledgeBase> getAllAiKnowledgeBase() {
        String sql = "SELECT id, category, keywords, answer, attachment_url, attachment_type FROM ai_knowledge_base ORDER BY id DESC";
    ...
    public void saveAiKnowledgeBase(AiKnowledgeBase kb) {
        if (kb.getId() == null) {
            String sql = "INSERT INTO ai_knowledge_base (category, keywords, answer, attachment_url, attachment_type) VALUES (?, ?, ?, ?, ?)";
            jdbcTemplate.update(sql, kb.getCategory(), kb.getKeywords(), kb.getAnswer(), kb.getAttachmentUrl(), kb.getAttachmentType());
        } else {
            String sql = "UPDATE ai_knowledge_base SET category = ?, keywords = ?, answer = ?, attachment_url = ?, attachment_type = ? WHERE id = ?";
            jdbcTemplate.update(sql, kb.getCategory(), kb.getKeywords(), kb.getAnswer(), kb.getAttachmentUrl(), kb.getAttachmentType(), kb.getId());
        }
    }
